...
public interface DetallePlanillaRepositoryDTO extends CrudRepository<DetallePlanillaDTO, Integer>{

	@Query("select dp from DetallePlanilla dp where dp.idPlanilla = ?1")
	public List<DetallePlanillaDTO> findByIdPlanilla(Integer planillaId);

	@Query("select v from DetallePlanilla v")
	//@Query("SELECT m FROM Movie m WHERE m.title LIKE %:title%")
	 List<DetallePlanillaDTO> findAll() ;
	
	@Query("select e from DetallePlanilla  e where e.idDetallePlanilla = :idDetalle")
	List<DetallePlanillaDTO> findAllById(Integer idDetalle) ;
	
	@Query("select e from DetallePlanilla  e where e.idPlanilla  = :idFactura")
	List<DetallePlanillaDTO> findByIdFactura(Long idFactura) ; 
	...
	List<DetallePlanillaDTO>findByIdPlanillas (List<Integer> idFacturas) ;
